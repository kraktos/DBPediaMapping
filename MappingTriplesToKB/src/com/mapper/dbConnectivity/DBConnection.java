...
public class DBConnection {
	...
	public Connection initDB() {
		try {
			...
			setConnection(DriverManager.getConnection(connectionURL + dbName,
	 ...
	 * @param start
	 */
	public static void performDBOperation(Statement statement,
			ResultSet resultSet, String testString, final String query) {
		...
		try {
			if (connection != null) {
				...
				if (!resultSet.next()) {
					DBTestClient.logger.info("no data");
				} else {
	
					do {
						DBTestClient.logger.info(resultSet.getString(1) + "\t\t"
								+ resultSet.getString(2));
					} while (resultSet.next());
				}
				
				DBTestClient.logger.info("\n  -----------------------      ------------------------------ \n");
			...
			DBTestClient.logger.info("PROCESS COMPLETED..FETCH TIME => " + (end - start)
					+ " ms.");
